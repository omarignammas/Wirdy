...
function CircularProgress({ value, size, strokeWidth, trackColor, progressColor, textColor, label, labelColor }: { value: number; size: number; strokeWidth: number; trackColor: string; progressColor: string; textColor: string; label: string; labelColor: string }) {
  ...
  return <View accessibilityLabel={`${label} ${normalized}%`} style={{ width: size, height: size }}><Svg width={size} height={size} style={{ transform: [{ rotate: '-90deg' }] }}><Circle cx={size / 2} cy={size / 2} r={radius} stroke={trackColor} strokeWidth={strokeWidth} fill="none" />{progressCircle}</Svg><View style={ringStyles.center}><Text style={[ringStyles.value, { color: textColor, fontSize: valueFontSize, lineHeight: valueFontSize + 4 }]}>{normalized}%</Text><Text style={[ringStyles.label, { color: labelColor, fontSize: labelFontSize }]} numberOfLines={1}>{label}</Text></View></View>
}

const ringStyles = StyleSheet.create({ center: { position: 'absolute', top: 0, right: 0, bottom: 0, left: 0, alignItems: 'center', justifyContent: 'center' }, value: { fontSize: 24, lineHeight: 28, fontWeight: '900', fontFamily: fontFamily('ui', 900), fontVariant: ['tabular-nums'] }, label: { fontSize: 10, fontWeight: '700', fontFamily: fontFamily('ui', 700), marginTop: 1, maxWidth: 78, textAlign: 'center' } })
