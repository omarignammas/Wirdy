...
function makeStyles(p: Palette, rtl: boolean) {
  ...
  return withAppFont(StyleSheet.create({
    ...
    mushafPage: { flex: 1, overflow: 'hidden', borderRadius: radius.xl, borderWidth: 1, borderColor: p.line, backgroundColor: dark === p ? p.elevated : '#FFFDF8', ...raised }, mushafHeader: { minHeight: 48, flexDirection: row, alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: spacing.md, borderBottomWidth: 1, borderBottomColor: p.line }, mushafMeta: { color: p.muted, fontSize: 10 }, mushafTitle: { color: p.primary, fontSize: 15, fontWeight: '700' }, verses: { padding: spacing.md }, verseRow: { position: 'relative', padding: spacing.sm, borderRadius: radius.sm }, verseSelected: { backgroundColor: p.primaryMuted }, quranText: { color: p.ink, lineHeight: 47, textAlign: 'right', writingDirection: 'rtl', fontFamily: Platform.OS === 'ios' ? 'Geeza Pro' : undefined }, verseMeta: { flexDirection: 'row', alignItems: 'center', alignSelf: 'flex-start', gap: 4, marginTop: 3 }, verseNumber: { minWidth: 25, color: p.gold, fontSize: 10, fontWeight: '600' },
