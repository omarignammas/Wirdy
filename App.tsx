...
import { fontFamily, withAppFont } from './src/font'
...
const light: Palette = {
  background: '#FFF6EF', surface: '#FFFDF9', elevated: '#F7EDE4', primary: '#0E4B36', primaryDeep: '#0A3D2C',
  primaryMuted: '#EAF4F0', ink: '#10251C', muted: '#756E66', line: '#DDD0C3', gold: '#D4A83B', danger: '#B54747',
  success: '#1D7A58', tab: 'rgba(255,253,249,0.88)', shadow: 'rgba(38,66,53,0.16)',
}

const dark: Palette = {
  background: '#0F1712', surface: '#18231E', elevated: '#24342C', primary: '#8FD3B1', primaryDeep: '#0E4B36',
  primaryMuted: '#233A30', ink: '#ECF7F1', muted: '#A9B5AF', line: '#31443B', gold: '#E7C968', danger: '#E0837F',
  success: '#8FD3B1', tab: 'rgba(24,35,30,0.78)', shadow: 'rgba(0,0,0,0.32)',
}
...
function MainApp({ language: initialLanguage, initialProfile, backendStatus, quranDatabase, themeMode, setThemeMode, darkMode, onSignOut }: { language: Language; initialProfile: WirdProfile; backendStatus: BackendStatus; quranDatabase: SQLiteDatabase; themeMode: MobileTheme; setThemeMode: (theme: MobileTheme) => void; darkMode: boolean; onSignOut: () => Promise<void> }) {
  ...
  function changeTab(next: TabId) {
    ...
    setTab(next)
    if (next !== 'reader') setReaderExpanded(false)
  ...
  const readerFullScreen = tab === 'reader' && readerExpanded
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      <View style={styles.app}>
        ...
        {notificationVisible && !readerFullScreen && <View style={styles.notificationToast}><Ionicons name="notifications" size={17} color={palette.primary} /><Text style={styles.notificationToastText}>{t.notificationMessage}</Text></View>}
        ...
        {tab === 'reader' && <ReaderScreen t={t} language={language} styles={styles} palette={palette} page={readerPage} verses={pageVerses} fontSize={fontSize} activeSession={activeSession} elapsed={elapsed} selectedVerse={selectedVerse} bookmarks={bookmarks} ayahNumbers={ayahNumbers} expanded={readerExpanded} onToggleExpanded={() => setReaderExpanded((value) => !value)} onSelectVerse={setSelectedVerse} onPageChange={setReaderPage} onStart={() => nextSession && startSession(nextSession)} onPause={async () => { if (activeSession) applySnapshot(await pauseMobileSession(activeSession.id)); setReaderExpanded(false); changeTab('home') }} onOpenSheet={setReaderSheet} onToggleBookmark={() => setBookmarks((current) => current.includes(selectedVerse) ? current.filter((item) => item !== selectedVerse) : [selectedVerse, ...current])} />}
        ...
        {!readerFullScreen && <TabBar active={tab} t={t} styles={styles} palette={palette} bottomInset={insets.bottom} onChange={changeTab} />}
...
function AppHeader({ language, title, avatarUri, notificationVisible, styles, palette, scrollY, showsInlineTitle, onToggleLanguage, onToggleNotifications }: { language: Language; title: string; avatarUri?: string; notificationVisible: boolean; styles: Styles; palette: Palette; scrollY: Animated.Value; showsInlineTitle: boolean; onToggleLanguage: () => void; onToggleNotifications: () => void }) {
  ...
  return <View style={styles.header}>
    ...
    <View style={styles.headerCopy}><Animated.Text style={[styles.headerTitle, { opacity: inlineOpacity }]} numberOfLines={1}>{title}</Animated.Text></View>
...
function ReaderScreen({ t, language, styles, palette, page, verses, fontSize, activeSession, elapsed, selectedVerse, bookmarks, ayahNumbers, expanded, onToggleExpanded, onSelectVerse, onPageChange, onStart, onPause, onOpenSheet, onToggleBookmark }: { t: typeof copy.ar | typeof copy.en; language: Language; styles: Styles; palette: Palette; page: number; verses: QuranVerse[]; fontSize: number; activeSession: WirdSession | null; elapsed: number; selectedVerse: number; bookmarks: number[]; ayahNumbers: boolean; expanded: boolean; onToggleExpanded: () => void; onSelectVerse: (verse: number) => void; onPageChange: (page: number) => void; onStart: () => void; onPause: () => void; onOpenSheet: (sheet: 'jump' | 'tafsir' | 'bookmarks' | 'complete') => void; onToggleBookmark: () => void }) {
  ...
  return <View style={[styles.readerScreen, expanded && styles.readerScreenExpanded]}>
    ...
    <View style={[styles.mushafPage, expanded && styles.mushafPageExpanded]}>
      <Pressable hitSlop={hitSlop} style={styles.mushafHeader} onPress={() => { void Haptics.selectionAsync(); onToggleExpanded() }}>
        <View style={styles.mushafHeaderEdge}><Text style={styles.mushafMeta}>{t.part} {juz}</Text></View>
        <Text style={styles.mushafTitle} numberOfLines={1}>{surahName}</Text>
        <View style={styles.mushafHeaderEdge}><Text style={styles.mushafMeta}>{t.page} {page}</Text><Ionicons name={expanded ? 'contract-outline' : 'expand-outline'} size={17} color={palette.primary} /></View>
      </Pressable>
      <ScrollView contentContainerStyle={[styles.verses, expanded && styles.versesExpanded]} showsVerticalScrollIndicator={false}>
        {verses.map((verse) => {
  ...
  if (selected) {
    ...
    return <ScrollView style={styles.screen} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.khatmaHero}>
        ...
        <View style={styles.heroProgress}><CircularProgress value={percent} size={118} strokeWidth={10} trackColor="rgba(255,255,255,0.18)" progressColor="#FFFFFF" textColor="#FFFFFF" label={t.complete} labelColor="rgba(255,255,255,0.66)" language={language} /><View style={styles.heroPosition}><Text style={styles.heroSurah}>{t.surahAlFurqan}</Text><Text style={styles.heroMeta}>{t.page} 362 · {t.part} 19</Text></View></View>
...
function TabBar({ active, t, styles, palette, bottomInset, onChange }: { active: TabId; t: typeof copy.ar | typeof copy.en; styles: Styles; palette: Palette; bottomInset: number; onChange: (tab: TabId) => void }) {
  ...
  const bottomOffset = bottomInset > 0 ? bottomInset + 6 : 9
  return <View style={[styles.tabBarShadow, { bottom: bottomOffset }]}>
...
function CircularProgress({ value, size, strokeWidth, trackColor, progressColor, textColor, label, labelColor, language }: { value: number; size: number; strokeWidth: number; trackColor: string; progressColor: string; textColor: string; label: string; labelColor: string; language: Language }) {
  ...
  const valueFontSize = Math.max(language === 'ar' ? 29 : 24, Math.round(size * (language === 'ar' ? 0.28 : 0.24)))
  const labelFontSize = size >= 100 ? language === 'ar' ? 13 : 11 : language === 'ar' ? 12 : 10
  const role = language === 'ar' ? 'arabic' : 'editorial'
  return <View accessibilityLabel={`${label} ${normalized}%`} style={{ width: size, height: size }}><Svg width={size} height={size} style={{ transform: [{ rotate: '-90deg' }] }}><Circle cx={size / 2} cy={size / 2} r={radius} stroke={trackColor} strokeWidth={strokeWidth} fill="none" />{progressCircle}</Svg><View style={ringStyles.center}><Text style={[ringStyles.value, { color: textColor, fontSize: valueFontSize, lineHeight: valueFontSize + 4, fontFamily: fontFamily(role, 900) }]}>{normalized}%</Text><Text style={[ringStyles.label, { color: labelColor, fontSize: labelFontSize, fontFamily: fontFamily(role, 700) }]} numberOfLines={1}>{label}</Text></View></View>
}

const ringStyles = StyleSheet.create({ center: { position: 'absolute', top: 0, right: 0, bottom: 0, left: 0, alignItems: 'center', justifyContent: 'center' }, value: { fontSize: 24, lineHeight: 28, fontWeight: '900', fontVariant: ['tabular-nums'] }, label: { fontSize: 10, fontWeight: '700', marginTop: 1, maxWidth: 78, textAlign: 'center' } })
...
function makeStyles(p: Palette, rtl: boolean) {
  ...
  const raised = { shadowColor: p.shadow, shadowOpacity: 1, shadowRadius: 18, shadowOffset: { width: 0, height: 9 }, elevation: 2 }
  const softRaised = { shadowColor: p.shadow, shadowOpacity: 0.78, shadowRadius: 12, shadowOffset: { width: 0, height: 6 }, elevation: 1 }
  return withAppFont(StyleSheet.create({
    safeArea: { flex: 1, backgroundColor: p.background }, app: { flex: 1, backgroundColor: p.background }, flexScreen: { flex: 1 }, screen: { flex: 1 }, scrollContent: { paddingHorizontal: spacing.lg, paddingTop: spacing.md, paddingBottom: 136 },
    header: { position: 'relative', flexDirection: row, alignItems: 'center', gap: spacing.sm, paddingHorizontal: spacing.lg, paddingTop: Platform.OS === 'android' ? 15 : spacing.md, paddingBottom: spacing.md }, headerHairline: { position: 'absolute', left: 0, right: 0, bottom: 0, height: StyleSheet.hairlineWidth, backgroundColor: p.line }, largeTitle: { color: p.ink, fontSize: rtl ? 43 : 38, lineHeight: rtl ? 52 : 44, fontWeight: '900', marginBottom: spacing.md, textAlign: align, writingDirection: writing },
    ...
    headerCopy: { flex: 1, minWidth: 0, justifyContent: 'center' }, headerActions: { flexDirection: row, alignItems: 'center', gap: 4 }, headerKicker: { color: p.muted, fontSize: rtl ? 13 : 11, fontWeight: '700', textAlign: align, writingDirection: writing }, headerTitle: { color: p.ink, fontSize: rtl ? 27 : 23, fontWeight: '900', lineHeight: rtl ? 36 : 31, textAlign: align, writingDirection: writing },
    headerButton: { position: 'relative', width: 44, height: 44, borderRadius: 22, borderWidth: 1, borderColor: p.line, backgroundColor: p.surface, alignItems: 'center', justifyContent: 'center', ...softRaised }, pressablePressed: { opacity: 0.72, transform: [{ scale: 0.97 }] }, pressableDisabled: { opacity: 0.48 }, notificationDot: { position: 'absolute', top: 7, right: 7, width: 8, height: 8, borderRadius: 4, borderWidth: 2, borderColor: p.surface, backgroundColor: p.gold },
    notificationToast: { position: 'absolute', zIndex: 20, top: 104, left: spacing.lg, right: spacing.lg, minHeight: 58, flexDirection: row, alignItems: 'center', gap: 8, paddingHorizontal: spacing.lg, borderRadius: radius.xl, borderWidth: 1, borderColor: p.line, backgroundColor: p.surface, shadowColor: p.shadow, shadowOpacity: 1, shadowRadius: 20, shadowOffset: { width: 0, height: 10 }, elevation: 3 }, notificationToastText: { flex: 1, color: p.ink, fontSize: rtl ? 18 : 15, lineHeight: rtl ? 25 : 20, fontWeight: '800', textAlign: align, writingDirection: writing },
    nextStrip: { minHeight: 82, flexDirection: row, alignItems: 'center', justifyContent: 'space-between', padding: spacing.md, borderRadius: radius.xl, borderWidth: 1, borderColor: p.line, backgroundColor: p.primaryMuted }, smallMuted: { color: p.muted, fontSize: rtl ? 15 : 12, lineHeight: rtl ? 21 : undefined, fontWeight: '700', textAlign: align, writingDirection: writing }, nextStripTitle: { marginTop: 2, color: p.primary, fontSize: rtl ? 25 : 20, lineHeight: rtl ? 32 : undefined, fontWeight: '900', textAlign: align }, timeChip: { minHeight: 50, flexDirection: row, alignItems: 'center', gap: spacing.xs, paddingHorizontal: spacing.md, borderRadius: radius.lg, backgroundColor: p.surface }, timeChipText: { color: p.primary, fontSize: rtl ? 24 : 19, lineHeight: rtl ? 30 : 24, fontWeight: '900', fontVariant: ['tabular-nums'] },
    heroCard: { marginTop: spacing.md, padding: spacing.lg, borderRadius: radius.xxl, backgroundColor: p.primaryDeep, shadowColor: p.shadow, shadowOpacity: 1, shadowRadius: 24, shadowOffset: { width: 0, height: 12 }, elevation: 3 }, heroTop: { flexDirection: row, alignItems: 'flex-start', justifyContent: 'space-between', gap: spacing.sm }, heroKicker: { color: 'rgba(255,255,255,0.72)', fontSize: rtl ? 15 : 12, fontWeight: '700', textAlign: align }, heroTitle: { maxWidth: rtl ? 280 : 260, marginTop: 2, color: '#FFFFFF', fontSize: rtl ? 35 : 29, fontWeight: '900', lineHeight: rtl ? 43 : 36, textAlign: align, writingDirection: writing }, partChip: { paddingHorizontal: spacing.sm, paddingVertical: spacing.sm, borderRadius: radius.md, backgroundColor: 'rgba(255,255,255,0.11)' }, partChipText: { color: '#FFFFFF', fontSize: rtl ? 14 : 11, lineHeight: rtl ? 19 : undefined, fontWeight: '800' },
    heroProgress: { flexDirection: row, alignItems: 'center', justifyContent: 'space-between', gap: rtl ? spacing.xxl : spacing.lg, marginTop: spacing.lg, paddingHorizontal: spacing.xs }, heroPosition: { flex: 1, minWidth: 0, marginStart: rtl ? 0 : spacing.md, marginEnd: rtl ? spacing.md : 0 }, heroSurah: { color: '#FFFFFF', fontSize: rtl ? 31 : 24, fontWeight: '900', lineHeight: rtl ? 39 : 32, textAlign: align }, heroMeta: { color: 'rgba(255,255,255,0.72)', fontSize: rtl ? 14 : 11, lineHeight: rtl ? 20 : undefined, fontWeight: '600', textAlign: align }, heroMetaRow: { flexDirection: row, alignItems: 'center', gap: spacing.xs, marginTop: spacing.sm }, heroActions: { flexDirection: row, gap: spacing.sm, marginTop: spacing.lg }, heroPrimary: { flex: 1, minHeight: 47, flexDirection: row, alignItems: 'center', justifyContent: 'center', gap: spacing.sm, borderRadius: radius.md, backgroundColor: '#FFFFFF' }, heroPrimaryText: { color: p.primaryDeep, fontSize: rtl ? 17 : 14, fontWeight: '800' }, heroGhost: { width: 47, minHeight: 47, alignItems: 'center', justifyContent: 'center', borderRadius: radius.md, borderWidth: 1, borderColor: 'rgba(255,255,255,0.28)' },
    sectionHeader: { flexDirection: row, alignItems: 'center', justifyContent: 'space-between', marginTop: spacing.lg, marginBottom: spacing.sm }, sectionTitle: { flex: 1, color: p.ink, fontSize: rtl ? 24 : 19, fontWeight: '900', lineHeight: rtl ? 32 : 26, textAlign: align }, sectionAction: { color: p.primary, fontSize: rtl ? 15 : 12, fontWeight: '800' },
    chartPanel: { minHeight: 236, padding: spacing.md, borderRadius: radius.xl, borderWidth: 1, borderColor: p.line, backgroundColor: p.surface, ...raised }, chartSelected: { alignSelf: rtl ? 'flex-end' : 'flex-start', paddingHorizontal: spacing.sm, paddingVertical: spacing.xs, borderRadius: radius.md, backgroundColor: p.primaryMuted }, chartSelectedText: { color: p.primary, fontSize: rtl ? 17 : 13, lineHeight: rtl ? 23 : undefined, fontWeight: '900', fontVariant: ['tabular-nums'] }, weekChart: { height: 188, flexDirection: row, alignItems: 'flex-end', justifyContent: 'space-between', gap: spacing.sm, marginTop: spacing.sm }, weekBarItem: { flex: 1, alignItems: 'center', gap: spacing.sm }, weekTrack: { width: 24, height: 152, overflow: 'hidden', justifyContent: 'flex-end', borderRadius: 12, backgroundColor: p.primaryMuted }, weekFill: { width: '100%', borderRadius: 12, backgroundColor: '#8AAE9F' }, weekLabel: { color: p.muted, fontSize: rtl ? 15 : 12, fontWeight: '700' }, weekLabelActive: { color: p.primary, fontWeight: '900' }, weekTotal: { color: p.muted, fontSize: rtl ? 17 : 13, fontWeight: '800', textAlign: 'center', fontVariant: ['tabular-nums'] },
    activityCard: { marginTop: spacing.md, padding: spacing.md, borderRadius: radius.xl, borderWidth: 1, borderColor: p.line, backgroundColor: p.surface, ...raised }, activityHeaderRow: { flexDirection: row, alignItems: 'center', justifyContent: 'space-between', marginBottom: spacing.md }, activityKicker: { color: p.muted, fontSize: 10, fontWeight: '700', textAlign: align, textTransform: 'uppercase' }, activityRangeLabel: { color: p.muted, fontSize: 10, fontWeight: '600', textTransform: 'uppercase' },
    activityBody: { flexDirection: row, alignItems: 'flex-start', gap: spacing.md }, activityStatCol: { width: rtl ? 134 : 124, gap: 3 }, activityValue: { color: p.ink, fontSize: rtl ? 40 : 34, lineHeight: rtl ? 46 : 38, fontWeight: '900', fontVariant: ['tabular-nums'], textAlign: align }, activityUnit: { color: p.muted, fontSize: rtl ? 17 : 14, fontWeight: '800' }, activityValueLabel: { color: p.muted, fontSize: rtl ? 13 : 10, fontWeight: '800', textAlign: align, textTransform: 'uppercase', marginBottom: spacing.sm }, activityChangeRow: { flexDirection: 'row', alignItems: 'center', gap: 3 }, activityChangeText: { fontSize: rtl ? 18 : 15, fontWeight: '900', fontVariant: ['tabular-nums'] }, activityChangeLabel: { color: p.muted, fontSize: rtl ? 12 : 10, fontWeight: '700', textAlign: align, textTransform: 'uppercase' },
    ...
    metricRow: { flexDirection: row, gap: spacing.sm, marginTop: spacing.md }, metric: { flex: 1, minHeight: rtl ? 138 : 124, padding: spacing.md, borderRadius: radius.xl, borderWidth: 1, borderColor: p.line, backgroundColor: p.surface, ...softRaised }, metricIcon: { width: 44, height: 44, borderRadius: radius.md, alignItems: 'center', justifyContent: 'center', backgroundColor: p.primaryMuted }, metricValue: { marginTop: spacing.sm, color: p.ink, fontSize: rtl ? 39 : 33, lineHeight: rtl ? 45 : 37, fontWeight: '900', fontVariant: ['tabular-nums'], textAlign: align }, metricLabel: { marginTop: 2, color: p.muted, fontSize: rtl ? 14 : 11, fontWeight: '700', lineHeight: rtl ? 19 : 15, textAlign: align },
    goalRow: { flexDirection: row, alignItems: 'center', justifyContent: 'space-between', marginTop: spacing.md, padding: spacing.lg, borderRadius: radius.xl, borderWidth: 1, borderColor: p.line, backgroundColor: p.surface, ...raised }, goalCopy: { flex: 1 }, goalTitle: { color: p.ink, fontSize: rtl ? 29 : 23, fontWeight: '900', lineHeight: rtl ? 38 : 29, textAlign: align, writingDirection: writing }, cardTitle: { color: p.ink, fontSize: rtl ? 21 : 17, fontWeight: '800', lineHeight: rtl ? 29 : 24, textAlign: align, writingDirection: writing }, cardMeta: { marginTop: 3, color: p.muted, fontSize: rtl ? 15 : 12, fontWeight: '600', lineHeight: rtl ? 22 : 19, textAlign: align, writingDirection: writing },
    smartCard: { flexDirection: row, alignItems: 'flex-start', gap: spacing.sm, marginTop: spacing.md, padding: spacing.md, borderRadius: radius.xl, borderWidth: 1, borderColor: p.line, backgroundColor: p.surface, ...raised }, smartIcon: { width: 44, height: 44, borderRadius: radius.md, alignItems: 'center', justifyContent: 'center', backgroundColor: p.primaryMuted }, smartCopy: { flex: 1 }, inlineActions: { flexDirection: row, alignItems: 'center', flexWrap: 'wrap', gap: spacing.sm, marginTop: spacing.sm }, smallButton: { minHeight: 44, justifyContent: 'center', paddingHorizontal: spacing.md, borderRadius: radius.md, backgroundColor: p.primaryMuted }, smallButtonPrimary: { backgroundColor: p.primary }, smallButtonText: { color: p.primary, fontSize: 13, fontWeight: '700' }, smallButtonTextPrimary: { color: '#FFFFFF' }, dismissText: { color: p.muted, fontSize: 12, fontWeight: '700' }, deleteText: { minHeight: 44, color: p.danger, fontSize: 13, fontWeight: '700', textAlignVertical: 'center' },
    spiritualCard: { flexDirection: row, alignItems: 'flex-start', gap: spacing.sm, marginTop: spacing.md, padding: spacing.md, borderRadius: radius.xl, borderWidth: 1, borderColor: p.line, backgroundColor: p.primaryMuted }, spiritualCopy: { flex: 1 }, spiritualTitle: { color: p.primary, fontSize: rtl ? 17 : 13, fontWeight: '700', textAlign: align }, spiritualText: { marginTop: 3, color: p.ink, fontSize: rtl ? 16 : 13, lineHeight: rtl ? 26 : 21, textAlign: align, writingDirection: writing },
    readerScreen: { flex: 1, paddingHorizontal: spacing.md, paddingBottom: 116 }, readerScreenExpanded: { paddingHorizontal: 0, paddingBottom: 0 }, readerStatus: { minHeight: 74, flexDirection: row, alignItems: 'center', justifyContent: 'space-between', padding: spacing.md, borderRadius: radius.xl, borderWidth: 1, borderColor: p.line, backgroundColor: p.primaryMuted }, compactPrimary: { minHeight: 46, flexDirection: row, alignItems: 'center', gap: spacing.xs, paddingHorizontal: spacing.md, borderRadius: radius.md, backgroundColor: p.primary }, compactPrimaryText: { color: '#FFFFFF', fontSize: rtl ? 16 : 13, fontWeight: '700' },
    focusBar: { minHeight: 74, flexDirection: row, alignItems: 'center', justifyContent: 'space-between', gap: spacing.sm, padding: spacing.md, borderRadius: radius.xl, backgroundColor: p.primaryDeep, ...raised }, focusLabel: { color: 'rgba(255,255,255,0.65)', fontSize: rtl ? 14 : 11, textAlign: align }, focusTitle: { marginTop: 2, color: '#FFFFFF', fontSize: rtl ? 19 : 15, fontWeight: '700', textAlign: align }, focusActions: { flexDirection: row, alignItems: 'center', gap: spacing.sm }, focusIcon: { width: 46, height: 46, borderRadius: radius.md, borderWidth: 1, borderColor: 'rgba(255,255,255,0.25)', alignItems: 'center', justifyContent: 'center' }, focusComplete: { minHeight: 46, flexDirection: row, alignItems: 'center', gap: spacing.xs, paddingHorizontal: spacing.md, borderRadius: radius.md, backgroundColor: '#FFFFFF' }, focusCompleteText: { color: p.primary, fontSize: rtl ? 15 : 12, fontWeight: '700' },
    readerToolbar: { flexDirection: row, gap: spacing.sm, marginVertical: spacing.md }, toolbarButton: { flex: 1, minHeight: 52, flexDirection: row, alignItems: 'center', justifyContent: 'center', gap: spacing.xs, borderRadius: radius.lg, borderWidth: 1, borderColor: p.line, backgroundColor: p.surface, ...softRaised }, toolbarIconWrap: { position: 'relative' }, toolbarLabel: { color: p.ink, fontSize: 11, fontWeight: '700' }, toolbarCount: { position: 'absolute', top: -9, right: -10, minWidth: 18, height: 18, paddingHorizontal: 4, borderRadius: 9, alignItems: 'center', justifyContent: 'center', backgroundColor: p.gold }, toolbarCountText: { color: '#FFFFFF', fontSize: 10, fontWeight: '900', fontVariant: ['tabular-nums'] },
    mushafPage: { flex: 1, overflow: 'hidden', borderRadius: radius.xl, borderWidth: 1, borderColor: p.line, backgroundColor: dark === p ? p.elevated : '#FFFDF8', ...raised }, mushafPageExpanded: { borderRadius: 0, borderWidth: 0, shadowOpacity: 0, elevation: 0 }, mushafHeader: { minHeight: 52, flexDirection: row, alignItems: 'center', justifyContent: 'space-between', gap: spacing.sm, paddingHorizontal: spacing.md, borderBottomWidth: 1, borderBottomColor: p.line }, mushafHeaderEdge: { minWidth: 70, minHeight: 44, flexDirection: row, alignItems: 'center', justifyContent: rtl ? 'flex-start' : 'flex-end', gap: spacing.xs }, mushafMeta: { color: p.muted, fontSize: 12, fontWeight: '800', fontVariant: ['tabular-nums'] }, mushafTitle: { flex: 1, color: p.primary, fontSize: 16, fontWeight: '700', textAlign: 'center' }, verses: { padding: spacing.md }, versesExpanded: { paddingHorizontal: spacing.lg, paddingTop: spacing.lg, paddingBottom: 84 }, verseRow: { position: 'relative', padding: spacing.sm, borderRadius: radius.sm }, verseSelected: { backgroundColor: p.primaryMuted }, quranText: { color: p.ink, lineHeight: 47, textAlign: 'right', writingDirection: 'rtl', fontFamily: Platform.OS === 'ios' ? 'Geeza Pro' : undefined }, verseMeta: { flexDirection: 'row', alignItems: 'center', alignSelf: 'flex-start', gap: 4, marginTop: 3 }, verseNumber: { minWidth: 30, color: p.gold, fontSize: 13, fontWeight: '900', fontVariant: ['tabular-nums'] },
    ...
    pageNavigation: { minHeight: 59, flexDirection: row, alignItems: 'center', justifyContent: 'center', gap: spacing.sm, paddingTop: spacing.sm }, pageButton: { width: 45, height: 45, alignItems: 'center', justifyContent: 'center', borderRadius: radius.md, borderWidth: 1, borderColor: p.line, backgroundColor: p.surface }, pageCenter: { minWidth: rtl ? 132 : 118, minHeight: 49, alignItems: 'center', justifyContent: 'center', borderRadius: radius.md, backgroundColor: p.primaryMuted }, pageCenterLabel: { color: p.muted, fontSize: rtl ? 12 : 9, fontWeight: '700' }, pageCenterValue: { color: p.primary, fontSize: rtl ? 21 : 16, lineHeight: rtl ? 27 : 20, fontWeight: '900', fontVariant: ['tabular-nums'] },
    pageIntro: { flexDirection: row, alignItems: 'center', gap: spacing.sm, marginBottom: spacing.lg }, pageIntroIcon: { width: 44, height: 44, flexShrink: 0, alignItems: 'center', justifyContent: 'center', borderRadius: radius.md, backgroundColor: p.primaryMuted }, pageIntroCopy: { flex: 1 }, pageIntroTitle: { color: p.ink, fontSize: rtl ? 29 : 23, fontWeight: '800', lineHeight: rtl ? 38 : 31, textAlign: align }, pageIntroText: { marginTop: 2, color: p.muted, fontSize: rtl ? 15 : 12, fontWeight: '500', lineHeight: rtl ? 23 : 18, textAlign: align, writingDirection: writing },
    summaryGrid: { flexDirection: row, flexWrap: 'wrap', overflow: 'hidden', marginBottom: spacing.md, borderRadius: radius.xl, borderWidth: 1, borderColor: p.line, backgroundColor: p.surface, ...raised }, summaryItem: { width: '50%', minHeight: rtl ? 96 : 86, justifyContent: 'center', paddingHorizontal: spacing.md, borderBottomWidth: 1, borderRightWidth: 1, borderColor: p.line }, summaryValue: { color: p.ink, fontSize: rtl ? 34 : 28, lineHeight: rtl ? 40 : 32, fontWeight: '900', fontVariant: ['tabular-nums'], textAlign: align }, summaryLabel: { color: p.muted, fontSize: rtl ? 14 : 11, fontWeight: '700', textAlign: align },
    segmented: { flexDirection: row, gap: 4, padding: 5, borderRadius: radius.lg, borderWidth: 1, borderColor: p.line, backgroundColor: p.elevated }, segment: { flex: 1, minHeight: 44, alignItems: 'center', justifyContent: 'center', paddingHorizontal: spacing.xs, borderRadius: radius.md }, segmentActive: { backgroundColor: p.surface, ...softRaised }, segmentText: { color: p.muted, fontSize: 12, fontWeight: '700' }, segmentTextActive: { color: p.primary },
    sessionCard: { flexDirection: row, alignItems: 'stretch', gap: spacing.sm, marginTop: spacing.md, padding: spacing.md, borderRadius: radius.xl, borderWidth: 1, borderColor: p.line, backgroundColor: p.surface, ...raised }, sessionTime: { width: rtl ? 88 : 78, minHeight: 82, alignItems: 'center', justifyContent: 'center', borderRadius: radius.lg, backgroundColor: p.primaryMuted }, sessionTimeValue: { color: p.primary, fontSize: rtl ? 27 : 22, lineHeight: rtl ? 33 : 27, fontWeight: '900', fontVariant: ['tabular-nums'] }, sessionTimeLabel: { marginTop: 2, color: p.muted, fontSize: rtl ? 13 : 10, fontWeight: '700' }, sessionBody: { flex: 1 }, sessionTitleRow: { flexDirection: row, alignItems: 'center', justifyContent: 'space-between', gap: spacing.xs }, sessionTitle: { flex: 1, color: p.ink, fontSize: rtl ? 22 : 18, fontWeight: '800', lineHeight: rtl ? 31 : 25, textAlign: align }, statusPill: { paddingHorizontal: spacing.sm, paddingVertical: spacing.xs, borderRadius: radius.md, backgroundColor: p.elevated }, statusActive: { backgroundColor: p.primaryMuted }, statusDone: { backgroundColor: p.primaryMuted }, statusText: { color: p.primary, fontSize: rtl ? 13 : 10, fontWeight: '700' },
    khatmaListHeader: { minHeight: 78, flexDirection: row, alignItems: 'center', gap: spacing.md, marginBottom: spacing.md }, khatmaListKicker: { color: p.muted, fontSize: rtl ? 15 : 12, fontWeight: '600', textAlign: align }, khatmaListTitle: { marginTop: 2, color: p.ink, fontSize: rtl ? 39 : 32, lineHeight: rtl ? 50 : 40, fontWeight: '800', textAlign: align }, khatmaAdd: { width: 54, height: 54, flexShrink: 0, alignItems: 'center', justifyContent: 'center', borderRadius: 27, backgroundColor: p.primaryDeep, shadowColor: p.shadow, shadowOpacity: 1, shadowRadius: 13, shadowOffset: { width: 0, height: 7 }, elevation: 2 },
    circlesSection: { marginBottom: spacing.lg }, circlesTitle: { color: p.ink, fontSize: rtl ? 22 : 17, fontWeight: '700', marginBottom: spacing.sm, textAlign: align }, circlesScroll: { flexDirection: row, gap: spacing.md, paddingBottom: 6 }, circleCard: { width: 156, height: 134, borderRadius: radius.xl, overflow: 'hidden', padding: spacing.md, justifyContent: 'flex-end', backgroundColor: p.primaryDeep, ...raised }, circleWatermark: { position: 'absolute', top: -10, insetInlineEnd: -10, opacity: 0.14 }, circleName: { color: '#FFFFFF', fontSize: rtl ? 20 : 16, fontWeight: '800', textAlign: align }, circleMeta: { marginTop: 2, color: 'rgba(255,255,255,0.7)', fontSize: rtl ? 14 : 11, fontWeight: '600', textAlign: align }, circleTime: { marginTop: 6, color: '#FFFFFF', fontSize: rtl ? 15 : 12, fontWeight: '700', textAlign: align }, circleAddCard: { width: 156, height: 134, borderRadius: radius.xl, borderWidth: 1, borderStyle: 'dashed', borderColor: p.line, alignItems: 'center', justifyContent: 'center', gap: spacing.xs, backgroundColor: p.surface, ...softRaised }, circleAddText: { color: p.muted, fontSize: rtl ? 15 : 12, fontWeight: '700', textAlign: 'center' },
    searchField: { minHeight: 56, flexDirection: row, alignItems: 'center', gap: spacing.sm, marginBottom: spacing.md, paddingHorizontal: spacing.md, borderRadius: radius.xl, borderWidth: 1, borderColor: p.line, backgroundColor: p.surface, ...softRaised }, searchInput: { flex: 1, color: p.ink, fontSize: 13, textAlign: align, writingDirection: writing },
    khatmaCard: { marginTop: spacing.md, padding: spacing.lg, borderRadius: radius.xl, borderWidth: 1, borderColor: p.line, backgroundColor: p.surface, ...raised }, khatmaCardTop: { flexDirection: row, alignItems: 'flex-start', justifyContent: 'space-between', gap: spacing.sm }, khatmaCardCopy: { flex: 1 }, khatmaCardTitle: { color: p.ink, fontSize: rtl ? 24 : 19, lineHeight: rtl ? 33 : 27, fontWeight: '700', textAlign: align, writingDirection: writing }, daysChip: { flexDirection: row, alignItems: 'center', gap: 4, paddingHorizontal: spacing.sm, paddingVertical: spacing.sm, borderRadius: radius.md, backgroundColor: p.primaryMuted }, daysChipText: { color: p.primary, fontSize: rtl ? 15 : 12, lineHeight: rtl ? 21 : 16, fontWeight: '900', fontVariant: ['tabular-nums'] }, khatmaProgressRow: { flexDirection: row, alignItems: 'center', gap: spacing.sm, marginTop: spacing.lg }, khatmaParts: { color: p.primary, fontSize: rtl ? 18 : 14, fontWeight: '900', fontVariant: ['tabular-nums'] }, khatmaPercent: { color: p.muted, fontSize: rtl ? 18 : 14, fontWeight: '900', fontVariant: ['tabular-nums'] }, khatmaTrack: { flex: 1, height: 9, overflow: 'hidden', borderRadius: 4.5, backgroundColor: p.primaryMuted }, khatmaFill: { height: '100%', borderRadius: 4.5, backgroundColor: p.primary }, khatmaEmpty: { alignItems: 'center', justifyContent: 'center', gap: 4, minHeight: 190, marginTop: spacing.md, padding: spacing.xl, borderRadius: radius.xl, borderWidth: 1, borderStyle: 'dashed', borderColor: p.line, backgroundColor: p.surface, ...softRaised },
    khatmaDetailHeader: { flexDirection: row, alignItems: 'center', gap: spacing.sm, marginBottom: spacing.md }, khatmaHero: { marginTop: spacing.md, padding: spacing.lg, borderRadius: radius.xxl, backgroundColor: p.primaryDeep, shadowColor: p.shadow, shadowOpacity: 1, shadowRadius: 24, shadowOffset: { width: 0, height: 12 }, elevation: 3 }, khatmaSchedule: { alignSelf: rtl ? 'flex-end' : 'flex-start', flexDirection: row, alignItems: 'center', gap: spacing.xs, marginTop: spacing.md, paddingHorizontal: spacing.sm, paddingVertical: spacing.sm, borderRadius: radius.md, backgroundColor: 'rgba(255,255,255,0.12)' }, khatmaScheduleText: { color: '#FFFFFF', fontSize: 12, fontWeight: '800', fontVariant: ['tabular-nums'] }, khatmaInvite: { minHeight: 44, flexDirection: row, alignItems: 'center', justifyContent: 'center', gap: spacing.sm, marginTop: spacing.sm }, khatmaInviteText: { color: 'rgba(255,255,255,0.76)', fontSize: 12, fontWeight: '800', fontVariant: ['tabular-nums'] },
    khatmaSessionRow: { minHeight: 78, flexDirection: row, alignItems: 'center', gap: spacing.md, borderBottomWidth: 1, borderBottomColor: p.line }, khatmaSessionIcon: { width: 48, height: 48, alignItems: 'center', justifyContent: 'center', borderRadius: radius.md, backgroundColor: p.primaryMuted }, khatmaSessionCopy: { flex: 1 }, memberStack: { overflow: 'hidden', paddingHorizontal: spacing.md, borderRadius: radius.xl, borderWidth: 1, borderColor: p.line, backgroundColor: p.surface, ...raised }, memberRow: { minHeight: 70, flexDirection: row, alignItems: 'center', gap: spacing.sm, borderBottomWidth: 1, borderBottomColor: p.line }, memberAvatar: { width: 46, height: 46, alignItems: 'center', justifyContent: 'center', borderRadius: 23, backgroundColor: p.primaryMuted }, memberAvatarText: { color: p.primary, fontSize: 17, fontWeight: '900', fontVariant: ['tabular-nums'] }, memberCopy: { flex: 1 },
    statsHero: { flexDirection: row, alignItems: 'center', justifyContent: 'space-between', gap: spacing.md, padding: spacing.lg, borderRadius: radius.xl, backgroundColor: p.primaryDeep, ...raised }, statsHeroCopy: { flex: 1 }, statsKicker: { color: '#FFFFFF', fontSize: rtl ? 28 : 22, fontWeight: '800', textAlign: align }, statsTitle: { marginTop: 5, color: 'rgba(255,255,255,0.67)', fontSize: rtl ? 15 : 12, lineHeight: rtl ? 23 : 19, textAlign: align, writingDirection: writing }, statsMetrics: { flexDirection: row, flexWrap: 'wrap', gap: spacing.sm, marginTop: spacing.md }, breakdownCard: { flexDirection: row, alignItems: 'center', justifyContent: 'space-between', marginTop: spacing.md, padding: spacing.lg, borderRadius: radius.xl, borderWidth: 1, borderColor: p.line, backgroundColor: p.surface, ...raised }, legendRow: { flexDirection: row, alignItems: 'center', gap: spacing.sm, marginTop: spacing.sm }, legendDot: { width: 8, height: 8, borderRadius: 4 },
    infoRow: { flexDirection: row, alignItems: 'center', gap: spacing.sm, minHeight: 69 }, infoCopy: { flex: 1 },
    streakCard: { marginTop: spacing.md, padding: spacing.md, borderRadius: radius.xl, borderWidth: 1, borderColor: p.line, backgroundColor: p.surface, ...raised }, streakHeader: { flexDirection: row, alignItems: 'center', justifyContent: 'space-between' }, streakHeaderLeft: { flexDirection: row, alignItems: 'center', gap: spacing.xs }, streakLabel: { color: p.ink, fontSize: rtl ? 18 : 14, fontWeight: '700' }, streakWindow: { color: p.muted, fontSize: rtl ? 15 : 12, fontWeight: '500' }, streakPills: { flexDirection: row, gap: spacing.xs, marginTop: spacing.md }, streakPill: { flex: 1, height: 26, borderRadius: radius.md, backgroundColor: p.elevated }, streakPillFilled: { backgroundColor: p.primary }, streakPillToday: { backgroundColor: p.gold },
    milestoneRow: { flexDirection: row, justifyContent: 'space-between', gap: spacing.sm }, milestoneItem: { flex: 1, alignItems: 'center', gap: spacing.xs }, milestoneBadge: { width: 52, height: 52, borderRadius: 26, alignItems: 'center', justifyContent: 'center', backgroundColor: p.elevated }, milestoneBadgeUnlocked: { backgroundColor: p.primaryMuted }, milestoneLabel: { color: p.muted, fontSize: rtl ? 12 : 9, fontWeight: '600', textAlign: 'center', lineHeight: rtl ? 16 : 13 },
    profileCard: { flexDirection: row, alignItems: 'center', gap: spacing.md, padding: spacing.lg, borderRadius: radius.xl, borderWidth: 1, borderColor: p.line, backgroundColor: p.surface, ...raised }, profileLarge: { width: 58, height: 58, borderRadius: 29, overflow: 'hidden', alignItems: 'center', justifyContent: 'center', backgroundColor: p.primaryDeep, borderWidth: 3, borderColor: p.primaryMuted }, profileLargeText: { color: '#FFFFFF', fontSize: rtl ? 30 : 25, fontWeight: '800' }, profileLargeImage: { width: '100%', height: '100%' }, profileCopy: { flex: 1 }, groupTitle: { marginTop: spacing.xl, marginBottom: spacing.sm, color: p.muted, fontSize: rtl ? 15 : 12, fontWeight: '800', textAlign: align, textTransform: 'uppercase' }, menuRow: { minHeight: 66, flexDirection: row, alignItems: 'center', gap: spacing.sm }, rowInset: { flex: 1, alignSelf: 'stretch', flexDirection: row, alignItems: 'center', justifyContent: 'space-between', borderBottomWidth: 1, borderBottomColor: p.line }, menuTitle: { flex: 1, color: p.ink, fontSize: rtl ? 22 : 17, fontWeight: '800', textAlign: align }, currentPlanCard: { minHeight: 94, flexDirection: row, alignItems: 'center', gap: spacing.sm, padding: spacing.md, borderRadius: radius.xl, borderWidth: 1, borderColor: p.line, backgroundColor: p.surface, ...raised },
    subpageHeader: { flexDirection: row, alignItems: 'center', gap: spacing.sm, marginBottom: spacing.md }, subpageCopy: { flex: 1 }, settingRow: { minHeight: 70, flexDirection: row, alignItems: 'center', gap: spacing.sm }, settingTitle: { flex: 1, color: p.ink, fontSize: rtl ? 19 : 15, fontWeight: '800', textAlign: align }, stepper: { flexDirection: 'row', alignItems: 'center', gap: spacing.sm }, stepButton: { width: 44, height: 44, alignItems: 'center', justifyContent: 'center', borderRadius: radius.md, backgroundColor: p.primaryMuted }, stepValue: { minWidth: rtl ? 60 : 54, color: p.ink, fontSize: rtl ? 22 : 17, lineHeight: rtl ? 28 : 22, fontWeight: '900', textAlign: 'center', fontVariant: ['tabular-nums'] }, settingChoice: { paddingVertical: spacing.md, borderBottomWidth: 1, borderBottomColor: p.line }, settingChoiceTitle: { marginBottom: spacing.sm, color: p.ink, fontSize: rtl ? 18 : 14, fontWeight: '800', textAlign: align }, dangerButton: { minHeight: 50, flexDirection: row, alignItems: 'center', justifyContent: 'center', gap: spacing.sm, marginTop: spacing.lg, borderRadius: radius.md, backgroundColor: `${p.danger}12` }, dangerText: { color: p.danger, fontSize: rtl ? 15 : 12, fontWeight: '800' },
    themeSetting: { gap: spacing.md, marginTop: spacing.sm, padding: spacing.md, borderRadius: radius.xl, borderWidth: 1, borderColor: p.line, backgroundColor: p.surface, ...raised }, themeSettingHeader: { flexDirection: row, alignItems: 'center', gap: spacing.sm }, themeSettingCopy: { flex: 1 },
    positionCard: { flexDirection: row, alignItems: 'center', gap: spacing.sm, padding: spacing.md, borderRadius: radius.xl, borderWidth: 1, borderColor: p.line, backgroundColor: p.surface, ...raised }, positionCopy: { flex: 1 }, planCard: { marginBottom: spacing.md, padding: spacing.md, borderRadius: radius.xl, borderWidth: 1, borderColor: p.line, backgroundColor: p.surface, ...raised }, planCardDisabled: { opacity: 0.62 }, planTop: { flexDirection: row, alignItems: 'center', gap: spacing.sm }, planNumber: { width: rtl ? 56 : 50, height: rtl ? 56 : 50, alignItems: 'center', justifyContent: 'center', borderRadius: radius.md, backgroundColor: p.primary }, planNumberText: { color: '#FFFFFF', fontSize: rtl ? 25 : 20, lineHeight: rtl ? 31 : 25, fontWeight: '900', fontVariant: ['tabular-nums'] }, planCopy: { flex: 1 }, planDetails: { flexDirection: row, flexWrap: 'wrap', gap: spacing.sm, marginTop: spacing.sm }, planDetail: { paddingHorizontal: spacing.sm, paddingVertical: spacing.xs, borderRadius: radius.md, backgroundColor: p.elevated, color: p.muted, fontSize: rtl ? 15 : 12, fontWeight: '800', fontVariant: ['tabular-nums'] }, addButton: { minHeight: 52, flexDirection: row, alignItems: 'center', justifyContent: 'center', gap: spacing.sm, borderRadius: radius.lg, borderWidth: 1, borderStyle: 'dashed', borderColor: p.primary, backgroundColor: p.primaryMuted }, addButtonText: { color: p.primary, fontSize: rtl ? 16 : 13, fontWeight: '700' }, saveBar: { minHeight: 56, flexDirection: row, alignItems: 'center', justifyContent: 'center', gap: spacing.sm, marginTop: spacing.md, borderRadius: radius.lg, backgroundColor: p.primary, ...softRaised }, saveBarText: { color: '#FFFFFF', fontSize: rtl ? 18 : 15, fontWeight: '800' },
    privacyBanner: { flexDirection: row, alignItems: 'flex-start', gap: spacing.sm, padding: spacing.md, borderRadius: radius.xl, borderWidth: 1, borderColor: p.line, backgroundColor: p.primaryMuted }, privacyCopy: { flex: 1 }, backupCard: { marginTop: spacing.md, padding: spacing.lg, borderRadius: radius.xl, borderWidth: 1, borderColor: p.line, backgroundColor: p.surface, ...raised }, backupIcon: { width: 47, height: 47, alignItems: 'center', justifyContent: 'center', borderRadius: radius.md, backgroundColor: p.primaryMuted }, backupTitle: { marginTop: spacing.md, color: p.ink, fontSize: rtl ? 24 : 19, fontWeight: '700', textAlign: align }, backupText: { marginTop: 5, color: p.muted, fontSize: rtl ? 15 : 12, lineHeight: rtl ? 24 : 19, textAlign: align, writingDirection: writing }, successBanner: { minHeight: 50, flexDirection: row, alignItems: 'center', justifyContent: 'center', gap: spacing.sm, marginTop: spacing.md, borderRadius: radius.lg, backgroundColor: p.primaryMuted }, successText: { color: p.success, fontSize: rtl ? 15 : 12, fontWeight: '600' },
    ...
    aboutHero: { alignItems: 'center', padding: spacing.lg, borderRadius: radius.xl, backgroundColor: p.primaryDeep, ...raised }, aboutMark: { width: 64, height: 64, alignItems: 'center', justifyContent: 'center', borderRadius: radius.xl, backgroundColor: '#FFFFFF' }, aboutMarkText: { color: p.primaryDeep, fontSize: rtl ? 36 : 31, fontWeight: '800' }, aboutTitle: { marginTop: spacing.md, color: '#FFFFFF', fontSize: rtl ? 28 : 22, fontWeight: '800', textAlign: 'center' }, aboutText: { marginTop: spacing.sm, color: 'rgba(255,255,255,0.67)', fontSize: rtl ? 15 : 12, lineHeight: rtl ? 24 : 19, textAlign: 'center' },
    legalSummary: { flexDirection: row, alignItems: 'flex-start', gap: spacing.sm, padding: spacing.lg, borderRadius: radius.xl, borderWidth: 1, borderColor: p.line, backgroundColor: p.primaryMuted }, legalSummaryIcon: { width: 42, height: 42, flexShrink: 0, alignItems: 'center', justifyContent: 'center', borderRadius: radius.md, backgroundColor: p.surface }, legalSummaryText: { flex: 1, color: p.ink, fontSize: rtl ? 16 : 13, fontWeight: '600', lineHeight: rtl ? 25 : 21, textAlign: align, writingDirection: writing }, legalSection: { paddingVertical: spacing.lg, borderBottomWidth: 1, borderBottomColor: p.line }, legalSectionTitle: { color: p.ink, fontSize: rtl ? 22 : 17, fontWeight: '700', lineHeight: rtl ? 30 : 24, textAlign: align, writingDirection: writing }, legalSectionBody: { marginTop: spacing.sm, color: p.muted, fontSize: rtl ? 15 : 12, fontWeight: '500', lineHeight: rtl ? 25 : 21, textAlign: align, writingDirection: writing }, legalContact: { flexDirection: row, alignItems: 'flex-start', gap: spacing.sm, marginTop: spacing.lg, padding: spacing.md, borderRadius: radius.xl, borderWidth: 1, borderColor: p.line, backgroundColor: p.surface, ...raised }, legalContactText: { flex: 1, color: p.muted, fontSize: rtl ? 14 : 11, lineHeight: rtl ? 22 : 19, textAlign: align, writingDirection: writing },
    tabBarShadow: { position: 'absolute', left: 7, right: 7, bottom: 9, minHeight: 84, borderRadius: 34, shadowColor: p.shadow, shadowOpacity: 1, shadowRadius: 28, shadowOffset: { width: 0, height: 12 }, elevation: 4 }, tabBar: { flex: 1, flexDirection: row, alignItems: 'center', gap: 2, paddingHorizontal: 7, paddingVertical: 8, borderRadius: 34, borderWidth: 1, borderColor: p.line, overflow: 'hidden' }, tabTint: { position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: p.tab }, tabItem: { flex: 1, minHeight: 66, alignItems: 'center', justifyContent: 'center' }, tabPillBase: { flexDirection: row, alignItems: 'center', justifyContent: 'center', gap: spacing.xs, minHeight: 52, minWidth: 52, paddingHorizontal: spacing.sm, borderRadius: 26 }, tabPillActive: { paddingHorizontal: spacing.md, backgroundColor: p.primaryMuted, shadowColor: p.shadow, shadowOpacity: 0.78, shadowRadius: 10, shadowOffset: { width: 0, height: 5 }, elevation: 1 }, tabLabel: { color: p.primary, fontSize: rtl ? 16 : 13, fontWeight: '800' },
    ...
    sheetHeader: { flexDirection: row, alignItems: 'center', gap: spacing.sm, paddingVertical: spacing.md }, sheetHeaderCopy: { flex: 1 }, sheetTitle: { color: p.ink, fontSize: rtl ? 23 : 18, fontWeight: '800', textAlign: align }, sheetSubtitle: { marginTop: 2, color: p.muted, fontSize: rtl ? 13 : 10, textAlign: align }, sheetClose: { width: 44, height: 44, alignItems: 'center', justifyContent: 'center', borderRadius: radius.md, backgroundColor: p.elevated }, sheetAction: { minHeight: 64, flexDirection: row, alignItems: 'center', gap: spacing.sm, borderBottomWidth: 1, borderBottomColor: p.line }, sheetActionIcon: { width: 44, height: 44, alignItems: 'center', justifyContent: 'center', borderRadius: radius.md, backgroundColor: p.primaryMuted }, sheetActionText: { flex: 1, color: p.ink, fontSize: rtl ? 18 : 14, fontWeight: '700', textAlign: align },
    fieldLabel: { marginTop: spacing.sm, marginBottom: 5, color: p.muted, fontSize: rtl ? 13 : 10, fontWeight: '700', textAlign: align }, textInput: { minHeight: rtl ? 54 : 48, flex: 1, paddingHorizontal: spacing.md, borderRadius: radius.md, borderWidth: 1, borderColor: p.line, color: p.ink, backgroundColor: p.elevated, textAlign: align, fontSize: rtl ? 17 : undefined }, formFields: { gap: spacing.xs }, jumpRow: { flexDirection: row, gap: spacing.sm }, modalPrimary: { minHeight: 50, alignItems: 'center', justifyContent: 'center', paddingHorizontal: spacing.lg, borderRadius: radius.md, backgroundColor: p.primary }, modalPrimaryText: { color: '#FFFFFF', fontSize: rtl ? 16 : 13, fontWeight: '800' }, modalGhost: { minHeight: 44, alignItems: 'center', justifyContent: 'center', marginTop: spacing.sm }, modalGhostText: { color: p.muted, fontSize: rtl ? 15 : 12, fontWeight: '700' }, modalActions: { flexDirection: row, gap: spacing.sm, marginTop: spacing.md }, tafsirBanner: { flexDirection: row, alignItems: 'center', gap: spacing.sm, padding: spacing.sm, borderRadius: radius.md, backgroundColor: p.primaryMuted }, tafsirBannerText: { color: p.primary, fontSize: rtl ? 13 : 10, fontWeight: '600' }, tafsirText: { marginTop: spacing.md, color: p.ink, fontSize: rtl ? 18 : 14, lineHeight: rtl ? 33 : 27, textAlign: align, writingDirection: writing }, emptyText: { paddingVertical: 25, color: p.muted, fontSize: rtl ? 15 : 12, textAlign: 'center' }, bookmarkRow: { minHeight: 65, flexDirection: row, alignItems: 'center', gap: spacing.sm, borderBottomWidth: 1, borderBottomColor: p.line }, bookmarkCopy: { flex: 1 }, completeIcon: { alignSelf: 'center', width: 61, height: 61, alignItems: 'center', justifyContent: 'center', borderRadius: 30.5, backgroundColor: p.primary }, completeTitle: { marginTop: spacing.md, color: p.ink, fontSize: rtl ? 22 : 17, fontWeight: '700', textAlign: 'center' }, completeText: { marginVertical: spacing.sm, color: p.muted, fontSize: rtl ? 14 : 11, textAlign: 'center' },
  }), rtl ? 'arabic' : 'editorial')
}
